...
public class TripleDES {
	...
	/**
	 * Gestiona la creación de una clave TripleDES
	 */
	public void doGenerateKey() {
		byte[] key = generateKey();
		if (key != null) {
			System.out.println("Clave generada:" + new String(Hex.encode(key)));
			Utils.instance().saveFile("tripledeskey", Hex.encode(key));
		}
	}
	...
	public void doEncrypt() {
		...
		if (text!=null) {
			...
			if (key != null) {
				...
				Utils.instance().saveFile("tripleencdes", Hex.encode(res));
			}
		} else {
	...
	/**
	 * Genera una Clave para el cifrado TripleDES a partir de un número aleatorio
	 * "seguro"
	 * 
