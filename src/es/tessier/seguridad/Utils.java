...
public class Utils {
	...
	public boolean saveConsoleToFile() {
		System.out.println("Introduzca el texto del archivo \n(Escriba 'fin' en una linea para finalizar)");
		String text = "", line = "";
		try {

			while (!line.equals("fin")) {

				line = Demos.scanner.nextLine().toLowerCase();
				if (!line.equals("fin")) {
					text = text + line + "\n";
				}
			}
	...
	public boolean saveFile(String ext, byte[] data) {
		try {
			System.out
					.print("Introduzca el nombre del archivo a grabar \n(se almacenara con la extension " + ext + "):");
			String fileName = Demos.scanner.nextLine();
